...
export default function PluginBuilder() {
  console.log("Rendering PluginBuilder component");
  
  const { user } = useAuth();
  const { tenant } = useTenant();
  
  console.log("Auth context:", { user: user ? "present" : "missing", userId: user?.id });
  console.log("Tenant context:", { tenant: tenant ? "present" : "missing", tenantId: tenant?.id });
  
  // Check if the hooks are properly loaded
  let canManagePlugins = false;
  try {
    canManagePlugins = useRolePermissions().canManagePlugins;
    console.log("Permission check result:", { canManagePlugins });
  } catch (error) {
  ...
  const savePlugin = async () => {
    ...
    try {
      console.log("Submitting plugin:", form);
  ...
  useEffect(() => {
    console.log("PluginBuilder mounted");
    return () => {
      console.log("PluginBuilder unmounted");
    };
  }, []);

  // Add a fallback if tenant is not loaded
  if (!tenant) {
    console.log("No tenant selected - showing fallback message");
    return (
      <div className="p-6">
        <h1 className="text-2xl font-bold mb-4">🔄 Loading workspace...</h1>
        <p>Please select a workspace or check your connection.</p>
      </div>
    );
  }
  ...
  if (!canManagePlugins) {
    console.log("User doesn't have permission - showing access denied");
    return (
      <div className="p-6">
